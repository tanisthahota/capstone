...
def parse_kb_entry(text: str) -> Tuple[int, List[str], List[str]]:
    ...
    # 1) Playbook: filename.yml
    # 2) Playbook-1: filename.yml
    # 3) Playbooks: filename1.yml, filename2.yml
...
def build_vars_dict(vars_list, ip, target):
    ...
    for v in vars_list:
        if v == "source_ip":
            ...
            # threat_name is not used by playbooks, skip it
            pass
        else:
            # keep unknown vars as empty string so ansible doesn't crash if optional
...
def mitigate(threat, ip, target, confidence):
    ...
    if "target_container_name" in sandbox_vars:
        sandbox_vars["target_container_name"] = SANDBOX_GATEWAY_NAME if is_gateway_playbook else f"sandbox-{prod_vars['target_container_name']}"
